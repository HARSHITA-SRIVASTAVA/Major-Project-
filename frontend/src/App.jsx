...
// ─── UPDATED DAILY CHECK-IN COMPONENT ─────────────────────────────────────────
function DailyCheckin({ onCheckin, events = [] }) {
  ...
  const handleAnalyzeText = async () => {
    ...
    try {
      const { daysUntilDeadline, isWeekend, upcomingEventType, upcomingEventTitle } = getCalendarContext();
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL ?? ""}/api/predict`, {
        ...
        body: JSON.stringify({
          ...
          upcoming_event_type: upcomingEventType,
          upcoming_event_title: upcomingEventTitle,
          questionnaire: {
            anxiety_level: 3,
            self_esteem: 3,
            sleep_quality: 3,
            academic_pressure: daysUntilDeadline !== null && daysUntilDeadline <= 3 ? 5 : 3,
            social_support: 3,
          },
        }),
      });
  ...
  const saveCheckinEntry = () => {
    if (!selected) return;
    const now = new Date();
    onCheckin({
      id: Date.now(),
      mood: selected,
      title: `AI Check-in — ${now.toLocaleDateString("en-IN", { day: "numeric", month: "short" })}`,
      body: note,
      tags,
  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontFamily: "'Lora', serif", fontSize: 26, color: C.text, fontWeight: 500, margin: 0 }}>Smart Daily Check-in</h1>
        <p style={{ fontSize: 14, color: C.textMuted, marginTop: 4 }}>Let our AI capture your entry sentiment dynamically</p>
      </div>
      <Card>
        <p style={{ fontFamily: "'Lora', serif", fontStyle: "italic", fontSize: 18, color: C.text, marginBottom: 8 }}>What's on your mind today?</p>
        <textarea
          value={note}
          onChange={e => setNote(e.target.value)}
          rows={4}
          placeholder="Type freely about your day, upcoming pressures, or how you feel... Then click Analyze below."
          style={{ width: "100%", boxSizing: "border-box", border: `1px solid ${C.border}`, borderRadius: 12, padding: 12, fontSize: 14, fontFamily: "'DM Sans', sans-serif", color: C.text, background: C.cream, resize: "none", outline: "none", marginBottom: "1rem", lineHeight: "1.5" }}
        />
        ...
        {analysis && (
          <div>
            ...
            <SubmitBtn onClick={saveCheckinEntry} disabled={!selected} color={C.sageDark}>
              Save AI-Verified Entry to Journal
            </SubmitBtn>
          </div>
        )}
      </Card>
...
function CalendarPage({ events, setEvents }) {
  ...
  const [form, setForm] = useState({ title: "", date: todayStr, type: "Exam", reminder: false });
  ...
  const handleAdd = () => {
    ...
    setEvents(ev => [...ev, {
      ...
      id: Date.now(),
      color: EVENT_COLORS[ev.length % EVENT_COLORS.length],
    }]);
    setForm({ title: "", date: form.date, type: "Exam", reminder: false });
    setShowForm(false);
  };
  ...
  const selectedDayStr = selectedDay ? dayToStr(selectedDay) : null;
        ...
        <Card>
          ...
          {showForm && (
            <div style={{ marginTop: "1rem", background: C.cream, borderRadius: 12, padding: 14, border: `1px solid ${C.border}` }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                <input
                  ...
                  style={{
                    ...
                    border: `1px solid ${C.border}`, fontSize: 13, background: C.white, color: C.text,
                ...
                <button
                  ...
                  style={{
                    ...
                    background: C.white, color: C.textMuted, fontSize: 12, cursor: "pointer",
              ...
              {form.date && (
                <p style={{ fontSize: 12, color: C.lavenderDark, margin: "0 0 10px" }}>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", minHeight: "100vh", position: "relative", zIndex: 1, width: "100%" }}>
        ...
        <main style={{ marginLeft: 240, flex: 1, padding: "2rem 2.5rem", minWidth: 0, boxSizing: "border-box" }}>
          ...
          {activeTab === "checkin" && (
            <DailyCheckin onCheckin={handleCheckin} events={events} />
          )}
          {activeTab === "journal" && (
